...
export async function validateVegetation(page) {return page.evaluate(async()=>{
 ...
 for(const {solver:s} of river.sections.sections.values()) {
  ...
  for(let i=0;i<100;i++){const o=i*4,h=roots[o+3];if(h===0)continue;trees++;minHeight=Math.min(minHeight,h);maxHeight=Math.max(maxHeight,h);if(flooded(roots[o],roots[o+1],roots[o+2]))wetTrees++;}
